...
instruction = schema_manager.generate_system_prompt(
    ...
    workflow_description=(
        ...
        "7. If a user asks to see the catalog or history, use get_fact_checks to retrieve the stored results. Proactively start with a brief conversational introduction paragraph. Then, represent the catalog list visually inside a structured A2UI Table component. Each catalog item MUST be rendered as a table row. NEVER output raw markdown bulleted lists or plain text blocks for the catalog. Always use a clean, visual Table component (containing columns for e.g., Verdict, Claim, Confidence, Date) to build a gorgeous database grid view.\n"
